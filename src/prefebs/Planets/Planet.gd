...
var max_shake = 1.0  # Amplitude da vibração (o quanto a tela vai tremer)
...
func start_shake(d_):
	var shake_amplitude = max_shake # <- criar uma lógica para velocidade de nave
	#Global.allNavs[Global.nav_select[Global.id]]["coeficiente"]
	var random_offset = Vector2(rand_range(-shake_amplitude+self.rect_position.x, shake_amplitude+self.rect_position.x), rand_range(-shake_amplitude+self.rect_position.y, shake_amplitude+self.rect_position.y))
	self.rect_position = random_offset
